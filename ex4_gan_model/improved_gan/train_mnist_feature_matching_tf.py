...
parser.add_argument('--learning_rate', type=float, default=0.0003)  # 学习率在1e-4
...
# 中间层在后面做匹配的效果会更好
def discriminator(input_img, reuse=False):
    # input_img = Input(batch_shape=(None, 3, 32, 32), dtype=im_dtype)
    with tf.variable_scope('Net_Dis', reuse=reuse) as scope:
        ...
        xx = layers.fully_connected(xx, num_outputs=250, activation_fn=None)
        #xx = layers.batch_norm(xx, is_training=is_training)
        xx0 = tf.nn.relu(xx)
        
        logits = layers.fully_connected(xx0, label_size, activation_fn=None)
...
output_before_softmax_unl, xx_unl = discriminator(x_unl, reuse=True)
output_before_softmax_gen, xx_gen = discriminator(gen_dat, reuse=True) 


def tf_log_sum_exp(xs):
  maxes = tf.reduce_max(xs, axis=1)
  return maxes + tf.log(tf.reduce_sum(tf.exp(xs - tf.expand_dims(maxes, 1)), axis=1))
...
# test error
output_before_softmax, xx_lab = discriminator(x_lab, reuse=True)
